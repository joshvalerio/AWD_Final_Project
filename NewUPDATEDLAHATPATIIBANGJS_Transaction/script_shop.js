...
const itemsToCheckout = ["Item 1", "Item 2", "Item 3"]; // Replace this with your actual items

// Store items in local storage when checkout button is clicked
document.getElementById("checkout-button").addEventListener("click", function() {
    localStorage.setItem("checkoutItems", JSON.stringify(itemsToCheckout));
});
document.addEventListener('DOMContentLoaded', function() {
  // Retrieve shopping cart items from local storage
  const shoppingCartItems = JSON.parse(localStorage.getItem('shoppingCart')) || [];

  // Function to update shopping cart in local storage
  function updateShoppingCart() {
      localStorage.setItem('shoppingCart', JSON.stringify(shoppingCartItems));
  }

  // Function to update transaction history in local storage
  function updateTransactionHistory() {
      // Retrieve existing transaction history from local storage
      const transactionHistory = JSON.parse(localStorage.getItem('transactionHistory')) || [];

      // Add items from the shopping cart to the transaction history
      shoppingCartItems.forEach(item => {
          transactionHistory.push(item);
      });

      // Update transaction history in local storage
  ...
  checkoutBtn.addEventListener('click', function() {
      // Update transaction history with items from shopping cart
  ...
  addItemBtn.addEventListener('click', function() {
      const newItem = 'New Item'; // Replace this with actual item data
