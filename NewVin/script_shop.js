...
const gotoPrev = () => current > 0 ? gotoNum(current - 1) : gotoNum(totalSlides - 1);

const gotoNext = () => current < totalSlides - 1 ? gotoNum(current + 1) : gotoNum(0);
...
const gotoNum = number => {
    ...
    // Adjust prev and next indices for the first and last slides
    if (next == totalSlides) {
        next = 0;
    }
...
// Add event listeners to the latest "Add to Cart" buttons in cart.html
const addToCartButtonsCart = document.querySelectorAll('.add-to-cart');
const latestProductIDs = [6, 7, 8]; // Product IDs for the latest items

if (addToCartButtonsCart) {
  addToCartButtonsCart.forEach((button, index) => {
    button.addEventListener('click', () => addToCart(latestProductIDs[index])); // Use the corresponding product ID
  });
}


// Add event listeners to the "Add to Cart" buttons inside the carousel
...
if (addToCartButtonsCarousel) {
  addToCartButtonsCarousel.forEach((button, index) => {
    button.addEventListener('click', () => {
      // Find the corresponding original "Add to Cart" button and trigger its click event
      const originalButton = document.querySelectorAll('.add-to-cart')[index + 6];
